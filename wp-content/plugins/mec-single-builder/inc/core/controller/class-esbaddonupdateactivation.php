...
class ESBAddonUpdateActivation
{
    ...
    public function AutomaticreActivation(){
        ...
        if ( !$reActivationOption ) {
            $verify_url = NS\PLUGIN_API_URL . '/activation/verify?category=mec_addons&license=' . $code . '&url=' . $url . '&item_id=' . $item_id;
    ...
    public function load_auto_update(){
        ...
        $MyUpdateChecker = \Puc_v4_Factory::buildUpdateChecker(
            add_query_arg(array('purchase_code' => $purchase_code, 'url' => $url,'id' => $product_id,'category' => 'mec_addons'), NS\PLUGIN_API_URL . '/updates/?action=get_metadata&slug='.$this->slug_string),
            $this->main_file_path,
            $this->slug_string
        );
